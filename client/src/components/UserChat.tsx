...
function UserChat({ chat, user }: userChat) {
  const { handleClickDeleteChat, handleSetCurrentChat, onlineUsers, notifications, markThisUserAsReadMessage } = useContext(ChatContext);
  ...
  return (
    <div className="group/item cursor-pointer hover:bg-blue-900 rounded transition duration-300 ease-in-out" 
      onClick={() => {
        handleSetCurrentChat(chat);
        if(userUnread?.length !== 0) {
          markThisUserAsReadMessage({userUnread, notifications})
        }
      }}
      >
